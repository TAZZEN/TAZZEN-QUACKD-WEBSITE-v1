    ...
    <main className="quackd-page" id="home">
      ...
      <section id="token" className="section-wrap scroll-mt-28 py-20 sm:py-32">
        <div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
          ...
          <Reveal delay={.1}><p className="max-w-lg text-base leading-7 text-[#806d88]">$QUACKD is the community layer around an open-source idea. No grand promises. No invented numbers. Just a shared symbol for people who think small robots with big personalities should exist.</p></Reveal>
        </div>
      ...
      <footer className="footer-shell border-t py-10">
        <div className="section-wrap flex flex-col gap-8 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2"><DuckMark small /><span className="font-mono text-sm tracking-[.17em] text-[#4c3459]">QUACKD</span></div><p className="mt-4 max-w-md text-xs leading-5 text-[#806d88]">A community meme token inspired by quackd. Built with curiosity, open source, and a slightly unreasonable belief in tiny robots.</p></div><div className="flex flex-col items-start gap-4 sm:items-end"><div className="flex gap-5 font-mono text-[10px] uppercase tracking-[.13em] text-[#806d88]"><button type="button" onClick={() => scrollToId('about')} className="focus-ring hover:text-[#b65c86]" data-testid="link-footer-about">About</button><button type="button" onClick={() => scrollToId('token')} className="focus-ring hover:text-[#b65c86]" data-testid="link-footer-token">Token</button><a href="https://github.com/rokbenko/quackd" target="_blank" rel="noreferrer" className="focus-ring hover:text-[#b65c86]" data-testid="link-footer-github">GitHub</a><a href="https://x.com/rokbenko" target="_blank" rel="noreferrer" className="focus-ring hover:text-[#b65c86]" data-testid="link-footer-x">X</a></div><div className="max-w-[440px] text-left font-mono text-[9px] leading-4 text-[#9d899f] sm:text-right">CONTRACT: {CONTRACT}<br />QUACKD is a community meme token inspired by quackd and is not official endorsement.</div></div></div>
      </footer>
    </main>
  );
}
